...
namespace TransXChange2GTFS_2 {
 class Program {
  ...
  static void Main(string[] args) {
   bankHolidayDates.AllBankHolidays = new List < string > (new string[] {
    "20180101",
    "20180330",
    "20180402",
    "20180507",
   ...
   foreach(NaptanStop stop in NaptanStops)
     stopIds.Add(stop.ATCOCode);
  ...
  static void convertTransXChange2GTFS(string filePath) {
     ...
     for (int j = 0; j < stopArray.Count; j++) {
      ...
      // For subsequent stops work out the time between stops
       ...
       if (timeGap.EndsWith("S") == true) {
        string cleanedTimeGap = timeGap.Split(new string[] {
         "PT"
        }, StringSplitOptions.None)[1].Replace("S", string.Empty);
        int timeIncrease = int.Parse(cleanedTimeGap);
        stopsTime = stopsTime.AddSeconds(timeIncrease);
       } else {
        string cleanedTimeGap = timeGap.Split(new string[] {
         "PT"
        }, StringSplitOptions.None)[1].Replace("M", string.Empty);
        int timeIncrease = int.Parse(cleanedTimeGap);
   ...
   if (InternalRoutesList.Count > 0) {
    ...
    if (processedRoutes.Contains(routeId)) {
     ...
     route.route_short_name = _txObject.Services.Service.Lines.Line.id;
 ...
 public class InternalRoute {
  ...
  public List < string > ExtraServiceDates {
   get;
   set;
  }
  public List < string > NoServiceDates {
   get;
   set;
  }
 ...
 public class BankHolidayDates {
  public List < string > AllBankHolidays {
   get;
   set;
  }
  ...
  public string LateSummerBankHolidayNotScotland {
   get;
   set;
  }
